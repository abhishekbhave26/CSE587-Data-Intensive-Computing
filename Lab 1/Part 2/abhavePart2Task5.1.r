...
#Replicate for 52 weeks

# Read the file and process it
data <-read.csv("WHO_NREVSS_Clinical_Labs2.csv")
data <- data[122:173,]
...
#png("clinical52.png",width=600,height=600)
#Plotting the plot
...
par(mar = c(5,5,2,5))
with(x,barplot(t(x),main="Influenza Positive Tests Reported To CDC by US Clinical Laboratories,\n National Summary, 2017-2018 52 Weeks Season ",
               xlab="Week", col=c("yellow","green"),ylim=c(0,25000),ylab="Number of Positive Specimens"))
text(cex=0.7,x=b-0.25,y=b-990.25,x1,xpd=TRUE,srt=60)
#axis(side=1,at=b,las=2,labels=x1,cex.axis=0.7)

par(new=T)

with(x,plot(x1, data$PERCENT.POSITIVE,axes=F,pch=16,xlab=NA,ylab=NA,cex=1.2,ylim=c(0,28),lwd=0))
axis(side=4)
...
legend("topright",c("A","B"),fill=c("yellow","green"),cex=0.65,bty="n")
